...
while True:
  ...
  if n % 100 == 0:
    state_storage = shelve.open(model_name, "n")
    errors, success = 0, 0
    for key in dir():
        try:
            if key == "errors" or key == "success":
                continue
            state_storage[key] = globals()[key]
            success += 1
        except TypeError:
            errors += 1
    print("errors in shelving:", errors, "success in shelving:", success)
    state_storage.close()
    pass
  ...
  if f_load == True:
      if load_model == True:
          ...
          state_storage = shelve.open(model_name)
          for key in state_storage:
              # print(key, state_storage[key])
              try:
                  globals()[key] = state_storage[key]
                  success += 1
              except:
                  errors += 1
          state_storage.close()
          print("s", success, "e", errors)
      f_load = False
